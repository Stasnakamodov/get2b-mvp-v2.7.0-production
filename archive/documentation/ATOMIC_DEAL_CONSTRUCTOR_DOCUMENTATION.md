...
```typescript
// ОБНОВЛЕННАЯ СТРУКТУРА ДАННЫХ (20.09.2025):
interface Step4Data {
  type: 'multiple';           // Всегда множественный выбор
  methods: string[];          // Массив доступных методов ['bank-transfer', 'p2p', 'crypto']
  payment_method: string;     // Выбранный метод
  auto_filled: boolean;       // Автозаполнение из каталога/эхо
  supplier_name?: string;     // Имя поставщика для автозаполнения
  supplier_data?: any;        // Данные поставщика
  catalog_source?: string;    // Источник: 'verified_supplier' | 'echo_data'
  user_choice: boolean;       // Пользователь сделал выбор
}
...
### **⚡ ПРИОРИТЕТНАЯ СИСТЕМА АВТОЗАПОЛНЕНИЯ (обновлено 24.09.2025):**

**НОВАЯ АРХИТЕКТУРА:** Трёхуровневая приоритизация с bidirectional sync между шагами 4↔5.

#### **🏆 Приоритет #1: selectedSupplierData (из каталога)**
Данные поставщика, выбранного через каталог - наивысший приоритет:

```typescript
// Приоритет 1: selectedSupplierData (самый актуальный источник)
if (selectedSupplierData) {
  if (method === 'bank-transfer' && (selectedSupplierData.bank_accounts?.length > 0 ||
      selectedSupplierData.payment_methods?.includes('bank-transfer'))) {
    hasSupplierData = true;
  }
  // ... аналогично для p2p и crypto
}
```

#### **🥈 Приоритет #2: manualData[4].methods (пользовательский выбор)**
Методы, выбранные пользователем или полученные из каталога:

```typescript
// Приоритет 2: Проверяем по методам из manualData[4] (из каталога)
if (manualData[4]?.methods && manualData[4].methods.includes(method)) {
  hasSupplierData = true;
}
```

#### **🥉 Приоритет #3: manualData[4].supplier_data (данные поставщика)**
Исторические данные поставщика как fallback:

```typescript
// Приоритет 3: Проверяем по доступным данным поставщика в manualData[4]
if (manualData[4]?.supplier_data) {
  const supplier = manualData[4].supplier_data;
  if (method === 'bank-transfer' && supplier.bank_accounts?.length > 0) {
    hasSupplierData = true;
  }
}
```
...
##### **1. Automatic Step Configuration**
Автоматическая установка `stepConfigs[5] = 'catalog'` при наличии autofill данных:
...
##### **2. Bidirectional Synchronization (Шаги 4↔5)**
Двусторонняя синхронизация между выбором метода оплаты и типом реквизитов:

```typescript
const handlePaymentMethodSelect = (method: string, supplier: any) => {
  // 🔄 Шаг 4 ← Обновляем выбранный метод
  setManualData(prev => ({ ...prev, 4: { type: 'single', method: method, selectedMethod: method }}))

  // 🔄 Шаг 5 ← Автоматически заполняем соответствующие реквизиты
  let requisitesData = getRequisitesForMethod(method, supplier);
  setManualData(prev => ({ ...prev, 5: requisitesData }))
}
```

##### **3. Enhanced Visual Highlighting**
Улучшенная визуальная подсветка кубиков с доступными методами оплаты:

```typescript
// Цветовое кодирование по методам с ring-эффектами
className={`... ${hasSupplierData
  ? (method === 'crypto' ? 'ring-2 ring-green-400 bg-green-500' :
     method === 'p2p' ? 'ring-2 ring-blue-400 bg-blue-500' :
     'ring-2 ring-orange-400 bg-orange-500')
  : 'bg-gray-400'
}`}
```

#### **🔄 Приоритет #4: Каталог поставщиков (Legacy)**
При добавлении товаров из каталога, система **приоритизирует свежие данные каталога** над историческими эхо данными:

```typescript
// НОВАЯ ЛОГИКА: Каталог данные ПЕРВЫМИ
...
fetch(`/api/catalog/verified-suppliers?search=${encodeURIComponent(firstProduct.supplier_name)}`)
  ...
  .then(data => {
    ...
    if (supplier) {
      // Приоритет каталогу - актуальные данные поставщика
      const step4Data = {
        type: 'multiple',
...
#### **🥈 Приоритет #2: Эхо данные (fallback)**
Эхо данные применяются только если поставщик **НЕ НАЙДЕН** в каталоге:

```typescript
// FALLBACK: Только если поставщик не найден в каталоге
.catch(() => {
  console.log('🔄 [ATOMIC] Поставщик не найден в каталоге - ищем эхо данные')
  // Логика эхо данных...
})
```

#### **🚫 Фильтрация платежных методов:**
- **Исключение "cash"** из фронтенда полностью
- **Нормализация** bank_transfer → bank-transfer
- **Дедупликация** методов для избежания дублей

```typescript
const normalizedMethods = (supplier.payment_methods || ['bank_transfer'])
  .map(method => method === 'bank_transfer' ? 'bank-transfer' : method)
  .filter(method => method !== 'cash')  // Убираем наличные
  .filter((value, index, self) => self.indexOf(value) === index)  // Дедупликация
```

#### **🎨 Визуальная индикация:**
- **Цветные кубики:** Показывают методы с автозаполнением от поставщика
- **Enhanced highlighting:** Ring-эффекты для доступных методов (green=crypto, blue=p2p, orange=bank)
- **Bidirectional sync:** Выбор на шаге 4 или 5 автоматически обновляет другой шаг
- **Серые кубики:** Показывают недоступные методы для ручного ввода
- **Все методы всегда видимы:** bank-transfer, p2p, crypto
...
## **📈 АРХИТЕКТУРНЫЕ ОБНОВЛЕНИЯ SEPTEMBER 2025**

### **🔄 Bidirectional Payment Synchronization**

Реализована полная двусторонняя синхронизация между шагами 4 и 5:

**Функциональность:**
- **Шаг 4 → Шаг 5:** Выбор метода оплаты автоматически заполняет соответствующие реквизиты
- **Шаг 5 → Шаг 4:** Выбор типа реквизитов обновляет метод оплаты

**Ключевые функции:**
```typescript
// Основная функция синхронизации
handlePaymentMethodSelect(method: string, supplier: any)

// Автоматическая установка конфигурации
useEffect(() => {
  // Проверка на наличие autofill данных
  // Автоматическое stepConfigs[5] = 'catalog'
}, [selectedSupplierData, manualData[4], stepConfigs[5]])

// Триадная проверка hasSupplierData
// Приоритет: selectedSupplierData > manualData[4].methods > manualData[4].supplier_data
```

### **🎨 Enhanced Visual System**

**Новая цветовая схема:**
- 🟢 **Crypto:** `ring-2 ring-green-400 bg-green-500`
- 🔵 **P2P:** `ring-2 ring-blue-400 bg-blue-500`
- 🟠 **Bank:** `ring-2 ring-orange-400 bg-orange-500`
- ⚫ **Недоступно:** `bg-gray-400`

**Ring-эффекты указывают на доступность autofill данных**

### **⚡ Performance Improvements**

**Оптимизированные запросы к БД:**
- Проверка через `selectedSupplierData` избегает лишних API вызовов
- Кеширование результатов приоритетной системы
- Ленивая загрузка данных поставщиков

### **🔧 Technical Debt Resolved**

**Исправленные проблемы:**
- ❌ JSX syntax errors в cube rendering
- ❌ Icon import conflicts (Wallet → Coins)
- ❌ API route broken imports (getSupabaseClient → supabase)
- ❌ hasSupplierData logic with else-if chains
- ❌ Inconsistent autofill detection

**Новые стандарты:**
- ✅ Independent payment method checking (no else-if)
- ✅ Proper TypeScript typing для supplier data
- ✅ Consistent error handling и logging
- ✅ IIFE syntax для conditional rendering

---

**🎯 КОНЕЦ ДОКУМЕНТАЦИИ: ATOMIC DEAL CONSTRUCTOR v1.1**

*Автор: GET2B Team*
*Дата создания: Январь 2025*
*Последнее обновление: Сентябрь 2025*
*Статус: Production Ready with Enhanced Features*

*Для последних обновлений см. репозиторий проекта*
```
